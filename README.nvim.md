...
iTerm2でlualineのセクション背景とセパレータの色が一致しない場合は、iTerm2の設定で背景色透過をオフにするか、Keep background colors opaqueを有効にする。

### 記号類の表示が崩れる


記号(→や※、罫線)が小さく表示されるのは、vim向けに調整済みのフォント（Bizin Gothic NFなど）はambiguous width character を予め半角幅に調整しているため。このようなフォントではambiwidthをデフォルト(single)にする。

vim向けに調整されてないフォントを使う場合はambiwidthをdoubleに設定する。

```lua
-- `config/nvim/lua/options.lua`
vim.opt.ambiwidth = "double"  -- vim向けに調整済みでないフォントを使う場合
``` 
